...
RUN rm -rf gpt4all
RUN mkdir chroma && mkdir model
RUN $HOME/.local/bin/poetry run python gpt_app_server/download_llm.py
# prepopulate vector database with some data in case can't upload
RUN $HOME/.local/bin/poetry run python gpt_app_server/extract_filing_langchain.py
